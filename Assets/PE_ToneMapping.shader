﻿Shader "PostEffect/ToneMapping"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                half4 col = SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearRepeat, input.texcoord);

                half3 acesIn = MulAcesInputMatrix(col.rgb);
                half3 acesMid = RRTAndODTFit(acesIn);
                half3 mapped = MulAcesOutputMatrix(acesMid);

                return half4(mapped, 1);
            }
